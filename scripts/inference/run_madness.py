...
weights_path = os.path.join(get_data_dir_path(), "models", survey_name + str(kl_weight))
LOG.info(f"Loading Model weights from {weights_path}")

simulation_path = os.path.join(madness_config["TEST_DATA_PATH"][survey_name], density)
results_path = madness_config["RESULTS_PATH"][survey_name]
...
for file_num in range(num_repetations):
    ...
    for field_num in range(len(blend.catalog_list)):
        ...
        madness_current_res["latent_rep"] = list(latent_reps[field_num][0:num_galaxies])
