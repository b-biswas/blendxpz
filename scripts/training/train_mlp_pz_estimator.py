...
dataset = "training"
data_path = data_path = os.path.join(BASE_DATA_PATH, blend_type + "_" + dataset, "photometry_data.pkl")
...
for dataset in ["training", "validation"]:
    data_path = os.path.join(BASE_DATA_PATH, blend_type + "_" + dataset, "photometry_data.pkl")
    with open(data_path, 'rb') as pickle_file:
        file_data = pickle.load(pickle_file)
    ...
    for filter in survey.available_filters: 
        ...
        data['x'][f"{filter}_phot_flux"] = (
                file_data[f"{filter}_phot_flux"].values 
                - norms['mu'][f"{filter}"]
...
# create model
mlp_estimator = create_mpl_estimator(
    num_filters=len(survey.available_filters)
)
